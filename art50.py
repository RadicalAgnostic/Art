...
Ver 46 - Random snake movement is now a separate function
Ver 47 - All movement patterns have independant functions
Ver 48 - Transition to circle printing functions
Ver 49 - Circle printing functions are now separate

Ver 50 - RGB TO HSV CONVERSIONS !!!!!!!

To do list: 
    
keep window from crashing after a full screen exit
find window size
Mirror movement
...
circle_data7 = [starting_pos7, 0, [255,255,255], 1]
...
def autoDraw(color, border_color, shade_range, starting_pos, movement = random_snake, length = 1000000000000000, circle_count = 0, last_color = 1, color_pattern = 0, color_direction = 1):
    ...
    if circle_count <= length:
        ...
        if color_pattern == bounce:
            ...
            if round(last_color) == color + shade_range:
        
                color_direction = -1
        
            elif round(last_color) == color:
            ...
            last_color = last_color + .07 * color_direction
...
def windowCheck(starting_pos, pos):
...
"""
---------------------------------
Here are the position funcitons
---------------------------------
"""


def randomSnake(circle_data):
    
    starting_pos = circle_data[0]
    ...
    starting_pos = windowCheck(starting_pos, pos)
    
    return starting_pos


def spray(circle_data):
...
"""
-----------------------------
Here are the color funcitons
-----------------------------
"""


def hsvToRgb(value_hsv):
    
    
    value_rgb = colorsys.hsv_to_rgb(value_hsv[0], value_hsv[1], value_hsv[2])

    value_rgb = [255 * value_rgb[0], 255 * value_rgb[1], 255 * value_rgb[2]]



    i = 0
    for value in value_rgb:
...
def rgbToHsv(value_rgb):
    
    value_hsv = colorsys.rgb_to_hsv(value_rgb[0], value_rgb[1], value_rgb[2])

    value_hsv = [1/255 * value_hsv[0], 1/255 * value_hsv[1], 1/255 * value_hsv[2]]
...
def scatter(circle_data):
    

    last_color = circle_data[2]
    
    color = rgbToHsv(last_color)
...
"""
------------------------------------
Here are the circle print funcitons
------------------------------------
"""


def oneCircle(circle_data):
    
    [starting_pos, circle_count, last_color, color_direction] = circle_data
    
    starting_pos = circle_data[0]
    circle_count = circle_data[1]
    last_color = circle_data[2]
    color_direction = circle_data[3]





    drawCircle(color, starting_pos, radius, border_size, white)


def worm(circle_data):
    ...
    first_pos = [starting_pos[0], starting_pos[1]]
    ...
    drawCircle(last_color, [first_pos[0] + (first_pos[0] - fifth_pos[0]), first_pos[1] + (first_pos[1] - fifth_pos[1]) ], fifth_radius,border_size,border_color)
    drawCircle(last_color, [first_pos[0] + (first_pos[0] - fourth_pos[0]), first_pos[1] + (first_pos[1] - fourth_pos[1]) ], fourth_radius,border_size,border_color)
    drawCircle(last_color, [first_pos[0] + (first_pos[0] - third_pos[0]), first_pos[1] + (first_pos[1] - third_pos[1]) ], third_radius,border_size,border_color)
    drawCircle(last_color, [first_pos[0] + (first_pos[0] - second_pos[0]), first_pos[1] + (first_pos[1] - second_pos[1]) ], second_radius,border_size,border_color)
    
    
    drawCircle(last_color,first_pos,first_radius,border_size,border_color)
    drawCircle(last_color,second_pos, second_radius,border_size,border_color)
    drawCircle(last_color,third_pos, third_radius,border_size,border_color)
    drawCircle(last_color,fourth_pos, fourth_radius,border_size,border_color)
    drawCircle(last_color,fifth_pos, fifth_radius,border_size,border_color)
...
while running:
    ...
    if play_Loop1 == True:
        ...
        circle_data1 = autoDraw(purple, black, 177, circle_data1[0], \
        ...
        circle_data2 = autoDraw(black, black, 100, circle_data2[0], \
                movement = long_worm, length = 10000000, circle_count = circle_data2[1],\
                    last_color = circle_data2[2])
            
        circle_data3 = autoDraw(white, black, 40, circle_data3[0], \
                movement = long_worm, length = 10000000, circle_count = circle_data3[1],\
                    last_color = circle_data3[2])
            
        circle_data4 = autoDraw(pink, black, 177, circle_data4[0], \
                movement = long_worm, length = 10000000, circle_count = circle_data4[1], \
    ...
    if play_Loop2 == True:
        ...
        circle_data7[0] = spray(circle_data7)
        
        circle_data7[2] = scatter(circle_data7)
